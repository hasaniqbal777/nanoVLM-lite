...
class BaseEvaluator(ABC):
    ...
    def __init__(self, model_path: str = "lusxvr/nanoVLM", device: str = "auto", resolution: int = None):
        ...
        if resolution is not None:
            ...
            if resolution != original_resolution:
                print(f"Updating model resolution from {original_resolution} to {resolution}")
                ...
                self._interpolate_positional_embeddings(original_resolution, resolution)
    ...
    def _interpolate_positional_embeddings(self, old_resolution: int, new_resolution: int):
        ...
        print(f"Interpolating positional embeddings: {old_num_patches} → {new_num_patches} patches")
        ...
        pos_embed = pos_embed.reshape(1, old_grid_size, old_grid_size, embed_dim)
        ...
        self.model.vision_encoder.patch_embedding.position_embedding = nn.Parameter(pos_embed)
        print(f"Updated positional embeddings to {new_grid_size}×{new_grid_size} grid")
            ...
            for idx, example in enumerate(tqdm(dataset, desc="MCQ Evaluation")):
                try:
                    ...
                    if idx < 3:
                        ...
                        print(f"Match: {'✓' if predicted_letter == correct_letter else '✗'}")
            ...
            for idx, example in enumerate(tqdm(dataset, desc="Open-Answer Evaluation")):
                try:
                    ...
                    if idx < 3:
                        ...
                        print(f"Model output: {output}")
                        print(f"Predicted: {predicted_answer}, Correct: {correct_answers[:3]}")
    ...
    def profile_model(self, test_image_path: str = "test/test_image.webp", test_query_path: str = "test/test_query.txt"):
        ...
        try:
            ...
            if Path(test_image_path).exists():
                manual_vit_flops = flop_counter.calculate_vit_flops_manual(image_path=test_image_path)
            else:
                print(f"Warning: Test image not found at {test_image_path}, using n_images=1")
                manual_vit_flops = flop_counter.calculate_vit_flops_manual(n_images=1)
            
            results["vit_manual_tflops"] = manual_vit_flops['vit_total_tflops']
            results["vit_manual_gflops"] = manual_vit_flops['vit_total_gflops']
            print(f"Vision encoder (manual): {manual_vit_flops['vit_total_tflops']:.4f} TFLOPs")
            
            # 2. Automatic ViT FLOP profiling
            if Path(test_image_path).exists():
                print("\n2. Profiling vision encoder FLOPs (automatic)...")
                auto_vit_flops = flop_counter.calculate_vit_flops_automatic(test_image_path)
                results["vit_auto_tflops"] = auto_vit_flops['vit_total_tflops']
                results["vit_auto_gflops"] = auto_vit_flops['vit_total_gflops']
                print(f"Vision encoder (automatic): {auto_vit_flops['vit_total_tflops']:.4f} TFLOPs")
            ...
            if Path(test_image_path).exists() and Path(test_query_path).exists():
                ...
                print(f"Generation (3 tokens): {gen_flops['generation_total_tflops']:.4f} TFLOPs")
            
            # Use vision encoder TFLOPs as the main estimate
            results["estimated_tflops_per_forward"] = results.get("vit_manual_tflops", 0)

#!/usr/bin/env python3
"""
Evaluation for nanoVLM on A-OKVQA.
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluation for nanoVLM on A-OKVQA")
    ...
    evaluator = Evaluator(
        args.model_path, args.device, resolution=args.resolution)
    ...
    print("\n" + "="*60)
    print(f"RUNNING EVALUATION ({mode.upper()})")
    print("="*60)
    ...
    if mode in ["mcq", "both"]:
        ...
        print(
            f"  Accuracy: {results['mcq_accuracy']*100:.2f}% ({results['mcq_correct']}/{results['mcq_total']})")
        print(
            f"  Avg Latency: {results['mcq_avg_latency']*1000:.2f} ms/sample")

    if mode in ["open-answer", "both"]:
        print()
        print("Open-Answer Evaluation:")
        print(
            f"  Accuracy: {results['oa_accuracy']*100:.2f}% ({results['oa_correct']}/{results['oa_total']})")

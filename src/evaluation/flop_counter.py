...
class FLOPCounter:
    def __init__(self, model_path: str = "lusxvr/nanoVLM", device: str = "auto", checkpoint_path: str = None):
        ...
        if checkpoint_path:
            ...
            if 'model_config' in checkpoint:
                ...
                print(
                    f"Loaded config from checkpoint (resolution: {self.cfg.vit_img_size})")
            else:
        ...
        resize_to_max_side_len = getattr(
            self.cfg, "resize_to_max_side_len", False)
    ...
    def calculate_generation_flops(
            ...
            with torch.no_grad():
                for _ in range(warmup_steps):
                    _ = self.model.generate(
                        tokens, img_tensor, max_new_tokens=1, greedy=True)
        ...
        with torch.no_grad():
            ...
            with flop_counter:
                _ = self.model.generate(
                    tokens, img_tensor, max_new_tokens=n_tokens, greedy=True)
            end_time = time.time()
...
def print_results(title: str, results: dict):
    ...
    for key, value in results.items():
        if isinstance(value, float):
            ...
            print(f"{key}: {value:,}" if isinstance(
                value, int) else f"{key}: {value}")
    print("="*60)
...
def main():
    ...
    print_results("1. Vision Encoder FLOPs (Manual Calculation)",
                  manual_results)

    # 2. Automatic ViT FLOPs
    auto_results = counter.calculate_vit_flops_automatic(args.image)
    print_results(
        "2. Vision Encoder FLOPs (Automatic Profiling)", auto_results)
    ...
    print_results(
        f"3. Generation FLOPs ({args.gen_tokens} tokens, NO warmup)", gen_no_warmup)
    ...
    print_results(
        f"4. Generation FLOPs ({args.gen_tokens} tokens, WITH {args.warmup} warmup steps)", gen_with_warmup)
    ...
    print(
        f"Difference:           {abs(manual_results['vit_total_tflops'] - auto_results['vit_total_tflops']):.4f}")
    print()
    print(
        f"Generation (no warmup):   {gen_no_warmup['generation_total_tflops']:.4f} TFLOPs")
    print(
        f"Generation (with warmup): {gen_with_warmup['generation_total_tflops']:.4f} TFLOPs")
    print(
        f"Warmup overhead:          {gen_no_warmup['latency_seconds'] - gen_with_warmup['latency_seconds']:.3f}s")
    print("="*60)
